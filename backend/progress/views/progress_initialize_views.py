# backend/progress/views/progress_initialize.py
...
class InitializeProgressView(APIView):
    ...
    ou pour réinitialiser la progression d'un utilisateur existant
    """
    ...
    def post(self, request):
        user = request.user
        logger.info(f"Initializing progress data for user {user.username}")
        ...
        for unit in units:
            ...
            logger.debug(f"Created {lessons_created} lesson progress entries for unit {unit.id}")
        ...
        if first_a1_unit:
            first_unit_progress = UserUnitProgress.objects.get(user=user, unit=first_a1_unit)
            if first_unit_progress.status == 'not_started':
                ...
                if first_lesson:
                    first_lesson_progress = UserLessonProgress.objects.get(user=user, lesson=first_lesson)
                    first_lesson_progress.status = 'in_progress'
                    first_lesson_progress.save()
        
        logger.info(f"Progress initialized: {units_created} new units, {units_existing} existing units")
        
        return Response({
            'success': True,
            'message': 'Progress data has been initialized',
            'stats': {
