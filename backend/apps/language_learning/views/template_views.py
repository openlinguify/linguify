"""
Vues pour les templates HTML/HTMX (héritées pour compatibilité)
Les fonctionnalités principales sont maintenant dans api_views.py
"""
...
def learning_interface(request):
    """Interface principale d'apprentissage - template HTML"""
...
def refresh_progress(request):
    ...
    try:
        ...
        context['user_progress'].update({
            ...
            'total_xp': getattr(learning_profile, 'total_time_spent', 0) * 10,
...
def progress_panel_partial(request):
    ...
    context = {
        'user_streak': user_streak,
        'course_units_raw': [{'id': i} for i in range(units_count)],
    }
